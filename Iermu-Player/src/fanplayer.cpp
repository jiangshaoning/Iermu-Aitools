...
static void* video_decode_thread_proc(void *param)
{
    ...
    while (!(player->player_status & PS_CLOSE)) {
        ...
        while (packet->size > 0 && !(player->player_status & (PS_V_PAUSE|PS_CLOSE))) {
			int len = ((packet->data[0] & 0xff) << 24) | ((packet->data[1] & 0xff) << 16) | ((packet->data[2] & 0xff) << 8) | ((packet->data[3] & 0xff));
			if (len == 5)
			{
				len = ((packet->data[9] & 0xff) << 24) | ((packet->data[10] & 0xff) << 16) | ((packet->data[11] & 0xff) << 8) | ((packet->data[12] & 0xff));
				if (packet->size != len + 13)
				{
					packet->size = len - 13;
				}
			}
